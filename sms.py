...
def toplevel_data(title,button_text,command):
    ...
    addressEntry.grid(row=4, column=1, pady=10, padx=10)
    ...
    genderEntry.grid(row=5, column=1, pady=10, padx=10)
    ...
    dobEntry.grid(row=6, column=1, pady=10, padx=10)
...
def delete_student():
    ...
    con.commit()
    messagebox.showinfo('Deleted',f'{content_id} is deleted successfully')
    query='select * from student'
    mycursor.execute(query)
    fetched_data=mycursor.fetchall()
    studentTable.delete(* studentTable.get_children())
...
def connect_database():
    def connect():
        ...
        try:
            ...
            mycursor.execute(query)
            query='create table student(id int not null primary key, name varchar(30),mobile varchar(12),email varchar(30),' \
        ...
        updatesstudentButton.config(state=NORMAL)
...
logo_image=PhotoImage(file='leftframe.png')
...
updatesstudentButton=ttk.Button(leftFrame,text='Updates Student',width=25,state=DISABLED,command=lambda : toplevel_data('Update Student','Update',update_data))
updatesstudentButton.grid(row=4,column=0,pady=20)
...
studentTable.column('Mobile No',width=400,anchor=CENTER)
